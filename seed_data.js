...
function importSeedData() {
    ...
    if (existing.length > 10) {
        const proceed = confirm(
            ...
            `Deseja importar mais ${SEED_SAIDAS.length} registros de demonstração?\n` +
            `Isso pode criar duplicatas.`
        );
        if (!proceed) {
            console.log('Importação de seed data cancelada pelo usuário.');
            return false;
        }
    }
    
    // Mesclar: adiciona seed data que não conflita por ID
    const existingIds = new Set(existing.map(s => s.id));
    const newItems = SEED_SAIDAS.filter(s => !existingIds.has(s.id));
    
    const merged = [...existing, ...newItems];
    merged.sort((a, b) => a.data.localeCompare(b.data) || (a.hora || '').localeCompare(b.hora || ''));
    
    localStorage.setItem('ct_saidas', JSON.stringify(merged));
    console.log(`Seed data importada: ${newItems.length} novos registros (total: ${merged.length})`);
    
    return true;
}

/* ---------- Auto-import: executar apenas na primeira vez ---------- */
(function() {
    const seedFlag = localStorage.getItem('ct_seed_imported');
    if (!seedFlag) {
        const ok = importSeedData();
        if (ok) {
            ...
            if (window.App && typeof App.loadData === 'function') {
                App.loadData().then(() => {
                    App.renderDashboard();
                    App.renderSaidas();
                    App.toast('Dados da planilha importados com sucesso!', 'success');
                });
            }
        }
    }
