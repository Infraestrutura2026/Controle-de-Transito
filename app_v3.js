/* ============================================================
   CONTROLE DE TRÂNSITO — VERSÃO 3.0 (OFFLINE-FIRST + SUPABASE)
   ============================================================
...
function getSupabaseKey() {
    return localStorage.getItem('ct_supabase_key') || SUPABASE_ANON_KEY_DEFAULT;
}
...
function initSupabase() {
    ...
    if (!url || !key) {
        console.warn('Supabase: URL ou chave não configurados. Use Configurações > Conexão Supabase para configurar.');
        return false;
    }
    try {
        supabaseClient = window.supabase.createClient(url, key);
        console.log('Supabase inicializado com sucesso (URL:', url, ')');
        return true;
    } catch (e) {
...
const App = {
    ...
    setupConfiguracoes() {
        ...
        const btnTestar = document.getElementById('btn-testar-supabase');
        const btnSalvarConn = document.getElementById('btn-salvar-supabase');
        ...
        if (btnTestar) btnTestar.addEventListener('click', () => this.testarConexaoSupabase());
        if (btnSalvarConn) btnSalvarConn.addEventListener('click', () => this.salvarConexaoSupabase());
    },
    ...
    loadConfigValues() {
        ...
        // Carregar valores de conexão Supabase
        const cfgUrl = document.getElementById('cfg-supabase-url');
        const cfgKey = document.getElementById('cfg-supabase-key');

        if (cfgUrl) cfgUrl.value = getSupabaseUrl();
        if (cfgKey) cfgKey.value = getSupabaseKey();
    ...
    // v3: Testar conexão com Supabase
    async testarConexaoSupabase() {
        const url = document.getElementById('cfg-supabase-url')?.value.trim();
        const key = document.getElementById('cfg-supabase-key')?.value.trim();

        if (!url || !key) {
            this.toast('Preencha a URL e a Chave Anon antes de testar.', 'error');
            return;
        }

        this.setLoading(true);
        try {
            const resp = await fetch(url + '/rest/v1/detentos?select=id&limit=1', {
                headers: {
                    'apikey': key,
            ...
            if (resp.ok) {
                ...
                this.toast('Erro na conexão: ' + resp.status + ' — ' + errText.substring(0, 120), 'error');
            }
        } catch (e) {
            this.toast('Falha ao conectar: ' + e.message, 'error');
        } finally {
            this.setLoading(false);
        }
    },

    // v3: Salvar conexão Supabase no localStorage e reinicializar cliente
    salvarConexaoSupabase() {
        if (!this.isAdmin()) {
            this.toast('Apenas administradores podem alterar a conexão.', 'error');
            return;
        }
        ...
        if (!url || !key) {
            this.toast('Preencha a URL e a Chave Anon antes de salvar.', 'error');
            return;
        }

        localStorage.setItem('ct_supabase_url', url);
        localStorage.setItem('ct_supabase_key', key);

        // Reinicializar cliente Supabase com novas credenciais
        const ok = initSupabase();
        if (ok) {
            this.toast('Conexão Supabase salva e inicializada com sucesso! ✅', 'success');
            // Tentar sincronizar dados imediatamente
            if (typeof this.syncAll === 'function') {
                this.syncAll();
            }
        } else {
            this.toast('URL/Chave salvas, mas falha ao inicializar o cliente. Verifique os valores.', 'warning');
        }
    },
        ...
        if (btnImport) {
            btnImport.addEventListener('click', () => {
                if (typeof SEED_SAIDAS === 'undefined') {
                    this.toast('Seed data não disponível. Verifique se seed_data_v3.js está carregado.', 'error');
                    return;
                }
