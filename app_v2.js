    ...
    renderDashboard() {
        ...
        if (locaisDiv) {
            if (!top5Locais.length) {
                ...
                locaisDiv.innerHTML = top5Locais.map(([local, count]) =>
                    `<div class="local-item" style="display:flex;justify-content:space-between;padding:4px 0;border-bottom:1px solid var(--border);">
                        <span>${this.esc(local)}</span>
                        <span class="badge" style="background:var(--primary);color:#fff;">${count}</span>
                    </div>`
                ).join('');
            }
        }
        ...
        if (ultimasDiv) {
            if (!recentes.length) {
                ...
                ultimasDiv.innerHTML = recentes.map(s => {
                    const tipoIcon = s.tipo === 'externa' ? '🔴' : '🟢';
                    return `<div class="recent-item" style="padding:4px 0;border-bottom:1px solid var(--border);">
                        <span>${tipoIcon} ${this.formatDate(s.data)} ${s.hora || ''} — ${this.esc(s.nome)} → ${this.esc(s.local)}</span>
                    </div>`;
                }).join('');
            }
        }
