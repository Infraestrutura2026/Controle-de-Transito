...
const App = {
    ...
    showMainScreen() {
        ...
        // Mostrar/ocultar menu Configurações para admins
    ...
    renderSaidas() {
        ...
        tbody.innerHTML = filtered.map(s => `
            <tr>
                ...
                <td>
                    ${isAdmin ? `<button class="btn-icon" title="Editar" onclick="App.editSaida('${s.id}')">✏️</button>` : ''}
    ...
    async exportarRelatorioPDF() {
        ...
        if (!filtered) {
            filtered = this.saidas.filter(s => s.data === data);
            const tipo = document.getElementById('rel-tipo').value;
            if (tipo) filtered = filtered.filter(s => s.tipo === tipo);
            filtered.sort((a, b) => a.hora.localeCompare(b.hora));
        }
