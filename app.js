...
const App = {
    ...
    imprimirRelatorio() {
        if (!this._lastRelatorioFiltered || !this._lastRelatorioFiltered.length) {
            this.toast('Gere o relatório primeiro antes de imprimir.', 'error');
            return;
        }
        const card = document.getElementById('relatorio-tabela-card');
        if (!card) return;
        const clone = card.querySelector('.card-body.table-container')?.cloneNode(true);
        if (!clone) return;
        // Remove botões e elementos irrelevantes da impressão
        const header = clone.querySelector('.relatorio-header-print');
        const footer = clone.querySelector('.relatorio-footer-print');
        const html = `
<!DOCTYPE html>
<html lang="pt-BR">
<head>
...
.relatorio-header-print { text-align: center; margin-bottom: 16px; }
.relatorio-header-print h2 { margin: 0; font-size: 18px; }
.relatorio-data { font-size: 13px; color: #555; margin: 4px 0 0; }
.relatorio-summary { display: flex; gap: 16px; margin-bottom: 16px; font-size: 13px; }
.rel-summary-item { display: flex; gap: 4px; }
.rel-summary-label { font-weight: 700; }
...
.badge-semiaberto { background: #dbeafe; color: #1d4ed8; }
.badge-fechado { background: #fee2e2; color: #b91c1c; }
.badge-aberto { background: #dcfce7; color: #15803d; }
.badge-tipo-externa { background: #e0f2fe; color: #0369a1; }
.badge-tipo-interna { background: #fef3c7; color: #92400e; }
.relatorio-footer-print { margin-top: 16px; font-size: 11px; color: #666; text-align: center; }
...
${clone.innerHTML}
        ...
        if (!win) { this.toast('Pop-up bloqueado. Permita pop-ups para imprimir.', 'error'); return; }
        win.document.open();
    ...
    async migrarLocalStorageParaSupabase() {
        if (!supabaseClient) {
            this.toast('Supabase não está configurado. Verifique SUPABASE_URL e SUPABASE_ANON_KEY.', 'error');
            return;
        }
        if (!this.isAdmin()) {
            this.toast('Apenas administradores podem executar a migração.', 'error');
            return;
        }

        const saidasLocal = JSON.parse(localStorage.getItem('ct_saidas') || '[]');
        const adminsLocal = JSON.parse(localStorage.getItem('ct_admins') || '[]');
        const auditLocal  = JSON.parse(localStorage.getItem('ct_audit') || '[]');

        const total = saidasLocal.length + adminsLocal.length + auditLocal.length;
        if (total === 0) {
            this.toast('Nenhum dado encontrado no localStorage para migrar.', 'warning');
            return;
        }

        const statusEl = document.getElementById('migracao-status');
        const textoEl  = document.getElementById('migracao-texto');
        if (statusEl) statusEl.style.display = '';

        let migrados = { saidas: 0, admins: 0, audit: 0, erros: 0 };
        this.setLoading(true);

        try {
            if (textoEl) textoEl.textContent = `Migrando ${saidasLocal.length} saídas...`;
            for (const s of saidasLocal) {
                try {
                    const dbRow = this.mapSaidaToDB(s);
                    const { error } = await supabaseClient.from('saidas').upsert(dbRow, { onConflict: 'id' });
                    if (error) { console.error('Erro migrando saída:', error); migrados.erros++; }
                    else migrados.saidas++;
                } catch (e) { console.error(e); migrados.erros++; }
            }

            if (textoEl) textoEl.textContent = `Migrando ${adminsLocal.length} administradores...`;
            for (const a of adminsLocal) {
                try {
                    const row = { nome: a.nome, matricula: a.matricula };
                    const { error } = await supabaseClient.from('admins').upsert(row, { onConflict: 'matricula' });
                    if (error) { console.error('Erro migrando admin:', error); migrados.erros++; }
                    else migrados.admins++;
                } catch (e) { console.error(e); migrados.erros++; }
            }

            if (textoEl) textoEl.textContent = `Migrando ${auditLocal.length} logs de auditoria...`;
            for (const a of auditLocal) {
                try {
                    const dbRow = this.mapAuditToDB(a);
                    const { error } = await supabaseClient.from('audit_logs').insert(dbRow);
                    if (error && error.code !== '23505') { console.error('Erro migrando audit:', error); migrados.erros++; }
                    else migrados.audit++;
                } catch (e) { console.error(e); migrados.erros++; }
            }

            if (textoEl) {
                textoEl.textContent = `✅ Migração concluída! Saídas: ${migrados.saidas} | Admins: ${migrados.admins} | Audit: ${migrados.audit} | Erros: ${migrados.erros}`;
            }
            this.toast(`Migração concluída! ${migrados.saidas + migrados.admins + migrados.audit} registros migrados.`, 'success');
            ...
            console.error(e);
            this.toast('Erro inesperado durante a migração.', 'error');
            if (textoEl) textoEl.textContent = '❌ Erro durante a migração. Verifique o console (F12).';
        } finally {
            this.setLoading(false);
        }
    },
